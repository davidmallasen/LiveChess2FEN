"""This module is responsible for predicting board configurations from images."""
...
def load_image(img_path: str, img_size: int, preprocess_func) -> np.ndarray:
    ...
    This function loads an image from its path. It is intended to be used
    for loading piece images.

    :param img_path: Image path.

    :param img_size: Size of the input image. Example: `224`.

    :param preprocess_func: Preprocessing fuction to apply to the input image.
...
def predict_board_keras(
    ...
    """Predict FEN string(s) from chessboard image(s) using Keras for inference.
    ...
    :param path: Path to the chessboard image or folder that contains chessboard
    images.
    ...
    corresponding to the chessboard image(s).
    ...
    :param previous_fen: FEN string of the previous board position.
        ...
        This parameter is only used when `path` points to a single image and `test` is
        `False`.

    :return: Predicted FEN string of the current board position for single-FEN prediction.
    ...
    def obtain_piece_probs_for_all_64_squares(pieces: list[str]) -> list[list]:
        predictions = []
        for piece in pieces:
...
def predict_board_onnx(
    ...
    """Predict FEN string(s) from chessboard image(s) using ONNXRuntime for inference.
...
def predict_board_trt(
    ...
    """Predict FEN string(s) from chessboard image(s) using TensorRT for inference.

    :param model_path: Path to the TensorRT engine with batch size 64 (ending with ".trt").
...
def predict_board(
    ...
    :param a1_pos: Position of the a1 square (`"BL"`, `"BR"`, `"TL"`, or `"TR"`)
    corresponding to the chessboard image.

    :param obtain_piece_probs_for_all_64_squares: Function that takes as input a
    length-64 list of paths to chess-piece images and returns a length-64 list of
    the corresponding piece probabilities (each element of the list is a length-13
    sublist that contains 13 piece probabilities).

        This parameter allows us to deploy different inference engines (Keras, ONNX,
        or TensorRT).

    :param board_corners: Length-4 list of the coordinates of the four board corners.

        The 4 corners are in the order of top left, top right, bottom right, and
        bottom left.

        If it is not `None` and the corner coordinates are accurate enough, the
        neural-network-based board-detection step is skipped (which means the total
        processing time is reduced).

    :param previous_fen: FEN string of the previous board position.

        If it is not `None`, it could significantly improve the accuracy of FEN
        prediction.

    :return: A pair formed by the predicted FEN string and the coordinates of the
    corners of the chessboard in the input image.
    """
...
def continuous_predictions(
    ...
    This function continuously monitors a folder and predicts the FEN strings for
    new jpg images added to the folder. The FEN string is printed out every time
    a prediction is completed. Note that this function does not return.

    :param path: Path to the folder that contains chessboard images.
...
def detect_input_board(
    ...
    This function takes as input a path to a chessboard image (e.g., "image.jpg")
    and stores the image that contains the detected chessboard in the "tmp" subfolder
    of the folder containing the board (e.g., "tmp/image.jpg").

    If the "tmp" folder already exists, the function deletes its contents.
    Otherwise, the function creates the "tmp" folder.
    ...
    :return: Length-4 list of the (new) coordinates of the four board corners detected.
    """
...
def obtain_individual_pieces(board_path: str) -> list[str]:
    ...
    :param board_path: Path to the chessboard image of interest.
        ...
        The image of the detected chessboard must be in the corresponding "tmp"
        folder (see `detect_input_board()`).
...
def time_predict_board(
    board_path, a1_pos, obtain_piece_probs_for_all_64_squares, previous_fen=None
):
    """Time the FEN-prediction process.

    This function predicts the FEN string from a chessboard and prints out the
    elapsed times during the prediction.

    The obtain_piece_probs_for_all_64_squares argument allows us to predict using different
    ...
    :return: Predicted FEN string corresponding to the input chessboard image.
    """
    total_time = 0
...
def print_fen_comparison(
    ...
    """Compare the predicted FEN with the correct fen and pretty-print the result.
    ...
    :param used_previous_fen: Whether the FEN string of the previous board position
    was used during the prediction.
    """
...
def read_correct_fen(fen_file: str) -> tuple[list, list, list]:
    """Read the correct FENs.

    :param board_path: Path to the file containing the correct FENs, a1-square
    positions, and (optionally) correct previous FENs.

    :return: Length-3 list of the correct FENs, a1-square positions, and correct
    previous FENs.

        Any `None` in the third sublist of the list represents an unknown previous
        board position.
    """
    fens = []
    a1_squares = []
...
def check_validity_of_fen(fen: str) -> bool:
    """Check the validity of the FEN string (assuming a standard physical chess set).
